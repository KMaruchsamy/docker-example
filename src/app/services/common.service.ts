...
export class CommonService {
  ...
  getStorage() {
    if (this.isPrivateBrowsing()) return (window.localStorage as any).__proto__;
  ...
  isAtomQuizBuilderEnable(): boolean {
    let atomQuizBuilderEnable: boolean = false;
    switch (location.hostname) {
      case this.getHostName(links.faculty.qa_new.server):
        atomQuizBuilderEnable = links.faculty.qa_new.atom_quiz_builder_enable;
        break;
      case this.getHostName(links.faculty.prod_new.server):
        atomQuizBuilderEnable = links.faculty.prod_new.atom_quiz_builder_enable;
        break;
      default:
        atomQuizBuilderEnable = links.faculty.atom_quiz_builder_enable;
        break;
    }
    return atomQuizBuilderEnable;
  }
  ...
  getOffsetInstitutionTimeZone(institutionId: number): number {
    ...
    if (institutions) {
      let selectedInstitution: any = _.find(JSON.parse(institutions), {
  ...
  getTimezone(institutionId: number): string {
    if (institutionId == undefined || institutionId === 0) return '';
  ...
  decryption(strToDecrypt) {
    ...
    let decryptedStr = CryptoJS.AES.decrypt(decodedString, key, {
      iv: iv,
  ...
  getEncryption(strToEncrypt) {
    let key = CryptoJS.enc.Base64.parse('MTIzNDU2NzgxMjM0NTY3OA==');
    let iv = CryptoJS.enc.Base64.parse('EBESExQVFhcYGRobHB0eHw==');
    let encryptedStr = CryptoJS.AES.encrypt(strToEncrypt, key, {
      iv: iv,
      mode: CryptoJS.mode.CBC,
      padding: CryptoJS.pad.Pkcs7
    }).toString();
