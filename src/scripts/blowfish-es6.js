...
*/
   // k:string;
    constructor(k) {
        ...
        this.key = this.k.length > 56 ? this.k.substr(0, 56) : this.k;
        let j = 0;
        for (let i = 0; i < 18; ++i) {
            const d = ((this.key.charCodeAt(j % this.key.length) * 256 + this.key.charCodeAt((j + 1) % this.key.length)) * 256 + this.key.charCodeAt((j + 2) % this.key.length)) * 256 + this.key.charCodeAt((j + 3) % this.key.length);
    ...
    encrypt(t) {
        for (let f = 0; f < t.length % 8; f++)
            t += '0';
        let enc = '';
        for (let f = 0; f < t.length; f += 8) {
            const l = t.substr(f, 4);
            const r = t.substr(f + 4, 4);
            let lcalc = l.charCodeAt(3) | l.charCodeAt(2) << 8 | l.charCodeAt(1) << 16 | l.charCodeAt(0) << 24;
            if (lcalc < 0) {
                lcalc = 4294967295 + 1 + lcalc;
            }
            let rcalc = r.charCodeAt(3) | r.charCodeAt(2) << 8 | r.charCodeAt(1) << 16 | r.charCodeAt(0) << 24;
            if (rcalc < 0) {
                rcalc = 4294967295 + 1 + rcalc;
            }
            this.xl_par = lcalc;
            this.xr_par = rcalc;
            this.encipher();
    ...
    wordescape(w) {
        let r = '';
        const m = new Array(this.wordbyte3(w), this.wordbyte2(w), this.wordbyte1(w), this.wordbyte0(w));
        for (let i = 3; i >= 0; i--) {
